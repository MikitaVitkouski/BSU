...
double GaussComposite(double a, double b, int K,
...
int main() {
    ...
    int K = 8; // amount of subintervals for [a;b] = [0.1;2]
    double result_composite = GaussComposite(0.1, 2.0, K, x_k, A_k);

    out << "Compound Gauss formula (n=4, K=" << K << "): " << result_composite << std::endl;

    double real_value{ 2.64577597114955282788543162027 }; // real value of integral f(x)dx on [0.1;2] to find real errors of the methods
    ...
    out << "Real error for compound Gauss formula: " << real_value - result_composite << std::endl;
